 ...
 * @param valueType
 */
export function registerEditorAttrCmp(
  klass: EditorPropertyComponent<any>,
  valueType?: string
) {
...
export class EditorManager {
  ...
  constructor(readonly config: EditorManagerConfig, store: StoreType) {
    ...
    this.message = new IframePostMessage(
      this.iframeElem.contentWindow as Window,
      config.originUrl
    );
  ...
  insterNode(
    editorData: Omit<EditorData, 'cmpType' | 'values' | 'id'> & {
      cmpType: keyof typeof SysComponents;
    },
    noticed?: boolean
  );
  insterNode(
    editorData: Omit<EditorData, 'values' | 'id' | 'cmpType'> & {
      cmpType: string;
    },
    noticed?: boolean
  );
