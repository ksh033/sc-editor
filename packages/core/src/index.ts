 ...
 * 编辑器属性组件
 */
export interface AppUIComponents<P, ValueTyps = SysComponentsEnumType>
  extends React.FC<P> {
  /**
   * 组件类型
   */
...
export function registerCmp(cmpType: SysComponentsEnumType, cmp: any): void;
export function registerCmp(cmpType: string, cmp: any): void;
...
export const RegAppUIComponent = <T extends {}>(
  cmp: AppUIComponents<T>
): FC<T> => {
  const Warp = (props: T) => {
